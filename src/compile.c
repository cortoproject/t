...
static corto_t_op* corto_t_addVal(corto_string start, corto_uint32 len, corto_t_expr *expr, corto_t_opKind flag, corto_t_compile_t *data);
...
static char* corto_t_numExpr(char *start, corto_t_expr *out, corto_t_compile_t *data) {
    ...
    ptr = corto_t_token(ptr, FALSE, TRUE, '\0', NULL, data);
    if (!ptr) {
        goto error;
    }

    corto_t_copySliceToString(num, (corto_t_slice){start, 1 + ptr - start});
...
static corto_int16 corto_t_parseObject(char *start, char *ptr, corto_t_expr *out, corto_t_compile_t *data) {
    ...
    if (*id == '/') {
        ...
        out->expr.object = corto_lookup(corto_lang_o, id);
    }
...
static char* corto_t_parseExpr(char *start, corto_t_expr *out, corto_t_compile_t *data) {
    ...
    if (ch == '"') {
        ...
        if (corto_t_parseObject(start, member ? member - 1: ptr, out, data)) {
            goto error;
        }
        if (member) {
    ...
    } else if ((ch == '\0') || ((ch == '.') && !isalpha(start[1]))) {
        ...
        ptr = corto_t_token(ptr, TRUE, TRUE, '\0', &member, data);
        if (ptr) {
            if ((ptr - start == 3) && !memcmp(start, "true", 1 + ptr - start)) {
                out->kind = CORTO_T_LITERAL;
                out->expr.literal.type = corto_type(corto_bool_o);
                out->expr.literal.value._bool = TRUE;
            } else if ((ptr - start == 4) && !memcmp(start, "false", 1 + ptr - start)) {
                out->kind = CORTO_T_LITERAL;
                out->expr.literal.type = corto_type(corto_bool_o);
            ...
            corto_t_err(data, "invalid expression", start);
            goto error;
        }
    } else {
        corto_t_err(data, "invalid expression", start);
...
static corto_t_op* corto_t_addVal(corto_string start, corto_uint32 len, corto_t_expr *expr, corto_t_opKind flag, corto_t_compile_t *data) {
    ...
    if (!expr) {
        if (!corto_t_parseExpr(start, &op->data.val.expr, data)) {
            goto error;
        }
    } else {
    ...
    corto_t_compile_t *data)
{
    ...
    if (arg) {
        if (!corto_t_addVal(NULL, 0, arg, CORTO_T_TOREG, data)) {
            goto error;
        }
    }
    ...
    corto_t_compile_t *data)
{
    ...
    if (!corto_t_addVal(NULL, 0, arg1, CORTO_T_TOREG, data)) {
        goto error;
    }
...
static corto_int16 corto_t_comp(
    ...
    corto_t_compile_t *data)
{
    return corto_t_comp_2arg(func, expr, comparator, NULL, data);
}
    ...
    corto_t_compile_t *data)
{
    ...
    /* If section doesn't close, a comparator argument is expected */
        ...
        end = corto_t_parseExpr(ptr, &expr, data);
        if (!end) {
            goto error;
        }
        ...
        if (*ptr == CORTO_T_CLOSE) {
            if (corto_t_comp_2arg(func, arg1, comparator, &expr, data)) {
                goto error;
            }
    ...
    corto_t_compile_t *data)
{
    ...
    char *ptr, *end = corto_t_parseExpr(start, &expr, data);
    if (!end) {
        goto error;
    }
    ...
    if (*ptr == CORTO_T_CLOSE) {
        if (!corto_t_func(func, &expr, data)) {
            goto error;
        }

    /* If section doesn't close, expect comparator */
    } else if ((ptr[0] == 'i') && (ptr[1] == 's')) {
        ptr = corto_t_section_comparator(ptr + 2, func, &expr, data);

    } else if (*ptr == '|') {
        corto_t_op *op;
        if (!(op = corto_t_func(func, &expr, data))) {
            goto error;
        }

        /* Write result of function to register */
        op->kind |= CORTO_T_TOREG;
        ptr = corto_t_section_filter(ptr + 1, data);
...
static char* corto_t_section(char *start, corto_t_compile_t *data) {
    ...
    if (*ptr == '>') {
        ...
        char *end = corto_t_parseExpr(ptr, &expr, data);
        if (!end) {
            goto error;
        } else {
         ...
         * operation or function for it */
        if (*ptr == CORTO_T_CLOSE) {
            /* Check for end of block */
            if ((id.len == 3) && !memcmp(id.ptr, "end", id.len)) {
                 if (data->sp) {
                     corto_t_popFunc(data);
                 } else {
            ...
            } else if (id.len && !((id.len == 1) && (*ptr == '.'))) {
                /* Check if a function with identifier exists */
                corto_id varId;
                corto_t_function f;
        ...
        /* If next token is a '|', this section is a filter */
            ...
            if (!corto_t_addVal(NULL, 0, &expr, CORTO_T_TOREG, data)) {
                goto error;
            }
...
static char* corto_t_sigil(char *start, corto_t_compile_t *data) {
    ...
    if (isspace(ch)) {
        ...
        if (!corto_t_addVal("", 0, NULL, CORTO_T_TOBUFF, data)) {
            goto error;
        }
    ...
    /* '$' is followed by an identifier */
        ...
        if (!corto_t_addVal(ptr, 1 + end - ptr, NULL, CORTO_T_TOBUFF, data)) {
            goto error;
        }
        ptr = end;
    }
